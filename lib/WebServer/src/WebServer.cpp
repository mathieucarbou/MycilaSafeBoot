...
void WebServer::requestAuthentication(HTTPAuthMethod mode, const char* realm, const String& authFailMsg) {
  ...
  if (mode == BASIC_AUTH) {
    ...
    sendHeader(
      ...
      AuthTypeDigest + String(F(" realm=\"")) + _srealm + String(F("\", qop=\"auth\", nonce=\"")) + _snonce + String(F("\", opaque=\"")) + _sopaque + String(F("\"")));
  }
  send(401, String("text/html"), authFailMsg);
}
...
bool WebServer::_handleRequest() {
  ...
  if (!handled) {
    send(404, "text/html", String("Not found: ") + _currentUri);
    handled = true;
  }
  if (handled) {
    _finalizeResponse();
